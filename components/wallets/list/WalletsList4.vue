...
export default {
  ...
  computed: {
    walletsIds () {
      const walletsIds = this.$store.getters['wallets/walletsVisibleIds']
...
.walllets
  ...
  .walletsList__toogle._alt(v-if="showToogle && $store.getters['wallets/walletsVisibleIds'].length > limit" @click="toogleWallets")
